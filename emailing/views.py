...
class AddSubscriber(BaseAPIView):
    ...
    def post(self, request):
        ...
        existing_emails = Subscriber.objects.filter(email__in=emails, mailing_list=mailing_list).values_list('email',
                                                                                                             flat=True)
...
class EmailSent(APIView):  # TODO Fa ca sa se salveze mesajele trimise in baza de date ca pe urma sa le afisezi aici
